...
<body>
    ...
    <form class="form-inline" action="<?=config("app.url"); ?>/crewskills/update/{{$crew_skill->id}}" method="post">
    @csrf
    <p>
    ...
    <p>
        ...
        value="{{$crew_skill->module_crew_id}}" required />
    </p>
    <p>
        <label for="name">Nazwa</label>
        <input type="text" name="name" id="name" size="25"
        value="{{$crew_skill->name}}" required />
    </p>
    <p>
